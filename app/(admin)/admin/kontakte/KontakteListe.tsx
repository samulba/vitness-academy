...
export function KontakteListe({ kontakte }: { kontakte: Kontakt[] }) {
  ...
  return (
    <div className="space-y-4">
      {/* Sticky Toolbar: Tabs + Suche + Filter-Pills.
          ...
          reicht top-0, da kein Topbar im Admin-Layout. */}
...
function KontaktZeile({ k }: { k: Kontakt }) {
  const tags = Array.isArray(k.role_tags) ? k.role_tags : [];
  const detailUrl = `/admin/kontakte/${k.id}`;
  ...
  return (
    <div className="group relative flex items-start gap-3 px-4 py-3 transition-colors hover:bg-[hsl(var(--brand-pink)/0.04)] sm:items-center sm:gap-4 sm:px-5 sm:py-4">
      {/* Karten-Klick-Layer (oeffnet Detail). Liegt UNTER den
          interaktiven Phone/Mail-Links (relative + z-Index). */}
      <Link
        href={detailUrl}
      ...
      <div className="relative z-10 min-w-0 flex-1 pointer-events-none">
        ...
        {/* Phone + Email als clickable Chips. Pointer-events ueber-
            schreibt parent damit tel:/mailto: funktioniert.
            Stacking: Mobile = Spalte, Desktop = Reihe. */}
        {(k.phone || k.email) && (
      ...
      {/* Chevron rechts: visueller Hint, dass der Eintrag klickbar ist */}
